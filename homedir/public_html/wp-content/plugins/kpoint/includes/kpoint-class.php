...
class KPoint{
	...
	private $balance = 0;
	private $user_id = 0;
	private $credit_meta_key = KPOINT__META_KEY_PREFIX . 'current_point';
	...
	public function display_balance(){
		return number_format($this->get_balance(), 0, ',', '.') . ' ' . KPOINT_UNIT_NAME;
	}

	public static function get_display_balance($amount){
		return number_format($amount, 0, ',', '.') . ' ' . KPOINT_UNIT_NAME;
	}
			...
			<table class="kp-logs">
				...
				<?php foreach ($logs as $log) : ?>
					<tr>
						...
						<td><?php echo number_format($log->amount, 0, ',', '.'); ?></td>
